...
function Complaints({ data, query, busy, run }: Context) {
  ...
  const contacts = filtered(rows(data, "contactSubmissions"), query);
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (
      await run(
  ...
  return (
    <>
      ...
      <h2 className="mt-8 text-lg font-extrabold">Contact form submissions</h2>
      <div className="mt-4 space-y-4">
        {contacts.map((contact) => (
          <Card key={string(contact.id)} className="p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <strong>{string(contact.subject)}</strong>
                <p className="mt-1 text-xs text-slate-500">
              ...
              <select
                disabled={busy}
                value={string(contact.status, "new")}
                onChange={(event) =>
                  run("contact.status", string(contact.id), {
                    status: event.target.value,
                  })
                }
                className="h-9 rounded-lg border px-2 text-xs capitalize"
              >
            ...
            <p className="mt-3 text-xs text-slate-500">
              {string(contact.category)} · {string(contact.phone)}
              {contact.order_number
                ? ` · Order ${string(contact.order_number)}`
                : ""}
            </p>
          </Card>
        ))}
