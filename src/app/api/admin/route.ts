...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "product.delete" && id) {
      ...
      if (old?.customer_id && old?.order_number) {
        await db.from("notifications").insert({
          user_id: old.customer_id,
          title: `Order ${old.order_number} updated`,
          body: `Your order status is now ${status.replaceAll("_", " ")}.`,
          type: "order_status",
          deep_link: `/track-order?order=${encodeURIComponent(old.order_number)}`,
        });
      }
      after = data;
      ...
      entityType = "shipment";
      const { data: shipmentOrder } = await db
        .from("orders")
      ...
      if (shipmentOrder) {
        await db.from("notifications").insert({
          user_id: shipmentOrder.customer_id,
          title: `Shipping update for ${shipmentOrder.order_number}`,
          body: `Shipment status is now ${values.status.replaceAll("_", " ")}${values.tracking_number ? ` with tracking ${values.tracking_number}` : ""}.`,
          type: "shipment_status",
          deep_link: `/track-order?order=${encodeURIComponent(shipmentOrder.order_number)}`,
        });
      }
      ...
      if (error) throw error;
      after = data;
    } else if (action === "contact.status" && id) {
      const status = text(payload, "status");
      ...
      if (old?.order_id)
        await db
          ...
          .eq("id", old.order_id);
      if (old?.order_id) {
        const { data: paymentOrder } = await db
          .from("orders")
          .select("customer_id,order_number")
          .eq("id", old.order_id)
          .maybeSingle();
        if (paymentOrder) {
          await db.from("notifications").insert({
            ...
            title: `Payment update for ${paymentOrder.order_number}`,
            body: `Payment status is now ${status.replaceAll("_", " ")}.`,
            type: "payment_status",
            deep_link: `/track-order?order=${encodeURIComponent(paymentOrder.order_number)}`,
          });
        }
      }
...
export async function GET(request: Request) {
  ...
  if (kind === "complaints") {
    ...
    rows = [
      ...rows.map((row) => ({ record_type: "account_complaint", ...row })),
      ...((contacts ?? []) as Record<string, unknown>[]).map((row) => ({
        record_type: "contact_submission",
        ...row,
      })),
    ];
  }
