    ...
    <StoreLayout>
      <div className="container-shell py-12">
        ...
        >
          ...
          <button className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 font-bold text-white">
            <Search size={17} /> Track securely
          </button>
        </form>
        {searched && (
          <div className="mx-auto mt-5 max-w-2xl rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-800">
            <strong>No configured order record was found.</strong>
            <p className="mt-1">
              Development-mode orders are not written to Supabase. Configure the
              database and retry with a persisted order.
            </p>
          </div>
        )}
      </div>
...
function Contact() {
  function send(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    toast.success("Message captured in development mode.");
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="text-xs font-bold sm:col-span-2">
              Inquiry category
              <select className="mt-2 h-11 w-full rounded-xl border px-3 text-sm">
          ...
          <button className="mt-5 flex h-12 items-center gap-2 rounded-xl bg-blue-600 px-6 font-bold text-white">
